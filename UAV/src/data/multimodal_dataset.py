...
REQUIRED_SOLVER_ALGORITHM = "constraint_aware_alternating_optimization"


def validate_multimodal_oracle_contract(
    data_dir: str, *, allow_legacy: bool = False
) -> Dict:
    ...
    if (
        metadata.get("solver_algorithm") != REQUIRED_SOLVER_ALGORITHM
        or not metadata.get("requires_oracle_feasible", False)
        or not metadata.get("generation_complete", False)
    ):
        if allow_legacy:
            return metadata
        raise ValueError(
            "dataset was not generated by the constraint-aware feasible Oracle: "
            f"{metadata_path}"
        )
    return metadata
